...
export default function Detail() {
    ...
    return (
        <div className={styles.container}>
            ...
            <h3 className={styles.temperaments}>{dog?.on_db ? dog?.temperaments : dog?.temperament}</h3>
            {dog?.id ? <h5 className={styles.height_weight}>{`Height: ${dog.on_db ? dog?.height : dog?.height.metric}`}</h5> : <></>}
            {dog?.id ? <h5 className={styles.height_weight}>{`Weight: ${dog.on_db ? dog?.weight : dog?.weight.metric}`}</h5> : <></>}
